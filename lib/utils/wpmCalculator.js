...
// Silence-removed speaking duration via offline Web Audio decoding.
// Returns null for codec variants unsupported by the browser — the server
// (ffmpeg inside Vercel Lambda) handles those via speakingDuration in response.
...
function detectSpeakingDuration(audioBuffer) {
  ...
  const MIN_SILENCE = Math.ceil(1.0 / 0.02);     // 50 windows = 1 s
