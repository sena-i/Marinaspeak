...
export async function saveSession(sessionData) {
  const { data, error } = await getSupabase()
    ...
    .insert({
      ...
      full_corrections: sessionData.fullCorrections || [],
      good_points: sessionData.goodPoints || null,
      coach_comment: sessionData.coachComment ? (typeof sessionData.coachComment === 'object' ? JSON.stringify(sessionData.coachComment) : sessionData.coachComment) : null,
      closing: sessionData.closing || null,
      repeated_mistakes: sessionData.repeatedMistakes || null,
      ...
      audio_file_path: sessionData.audioFilePath || null
    })
    .select()
    .single();
...
export async function uploadAudio(studentId, sessionId, audioBuffer, mimeType) {
  ...
  const contentType = (mimeType === 'audio/x-m4a' || mimeType === 'audio/m4a')
    ? 'audio/mp4'
    : mimeType;

  const filePath = `${studentId}/${sessionId}.${ext}`;
...
export async function getAudioUrl(filePath) {
  const { data, error } = await getSupabase()
    .storage
    .from('audio')
